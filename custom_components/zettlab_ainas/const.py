...
# UDP broadcast discovery (native ZettOS protocol, local network only). The
# device listens on this port; the exact probe payload used by the official app
# is NOT yet decoded, so DISCOVERY_PROBE is a placeholder — until it is captured,
# discovery yields nothing and the user falls back to manual host entry.
