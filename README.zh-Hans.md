...
- 📊 **系统监控** —— CPU 使用率与温度、NPU/GPU 使用率、内存使用率 / 已用 / 空闲 / 缓存 / 总量，以及逐网口吞吐。
- 💾 **存储** —— 每个存储池的容量 / 已用 / 空闲 / 使用率、RAID 异常状态，以及逐磁盘温度和读写吞吐。
- 🌬️ **风扇** —— 监测每路风扇转速并切换风扇模式。
...
| 平台 | 实体 | 来源 |
|------|------|------|
| `sensor` | CPU 温度/使用率、NPU·GPU 使用率、内存指标、风扇转速 | 实时监控 |
| `sensor` | 逐网口上传/下载吞吐 | 实时监控 |
| `sensor` | 存储池容量/已用/空闲/使用率、逐磁盘温度与 I/O | 存储池 + 实时监控 |
| `sensor` | 上次启动(诊断) | 设备信息 |
